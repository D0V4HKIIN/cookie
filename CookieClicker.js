...
var rng = 10;
	...
	setInterval(
		function change_speed(){
			...
			document.getElementById("mar").scrollAmount = rng;
		}, 1000);
	...
	function cookies_farm(){
		if (cookies_nb - 100*Number(document.getElementById("multiplier").value) >= 0){
			cookies_nb = Math.round((cookies_nb - 100*Number(document.getElementById("multiplier").value))*10)/10;
			document.getElementById("cookies_show").innerHTML = cookies_nb;
	...
	function cookies_usine(){
		if (cookies_nb - 1000*Number(document.getElementById("multiplier").value) >= 0){
			cookies_nb = Math.round((cookies_nb - 1000*Number(document.getElementById("multiplier").value))*10)/10;
			document.getElementById("cookies_show").innerHTML = cookies_nb;
	...
	function cookies_god(){
		if (cookies_nb - 1000000000*Number(document.getElementById("multiplier").value) >= 0){
			cookies_nb = Math.round((cookies_nb - 1000000000*Number(document.getElementById("multiplier").value))*10)/10;
			document.getElementById("cookies_show").innerHTML = cookies_nb;
		...
		setInterval(
			function cookie_loop(){
				cookies_nb = cookies_nb + (cookies_per_sec/10);
				cookies_nb = Math.round(cookies_nb*10)/10;
				document.getElementById("cookies_show").innerHTML = cookies_nb;
		}, 100);
